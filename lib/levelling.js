module.exports = {
    ...
    getLevelDetails(level, xp, multiplier = global.multiplier || 1) {
        const currentLevel = this.findLevel(xp, multiplier)
        const {min: nextMin, max: nextMax} = this.xpRange(currentLevel + 1, multiplier)
        const remainingXP = nextMax - xp
        return {
            level: currentLevel,
            progress: (xp - nextMin) / (nextMax - nextMin) * 100,
            remainingXP
        }
    }
}
