...
let handler = async (m, {
  ...
  if (command == 'ffstalk') {
    ...
    try {
      let ff = await fetch(`https://api.betabotz.eu.org/fire/stalk/ff?id=${text}&apikey=${lann}`).then(res => res.json());
    ...
    if (command == 'mlstalk') {
    if (text.length < 2) throw `Example: ${usedPrefix + command} 2480197|2001`;
    let [id, server] = text.split('|');
    try {
      let ml = await fetch(`https://api.betabotz.eu.org/fire/stalk/ml?id=${id}&server=${server}&apikey=${lann}`).then(res => res.json());
  ...
  if (command == 'supersusstalk') {
    ...
    try {
      let sus = await fetch(`https://api.betabotz.eu.org/fire/stalk/supersus?id=${text}&apikey=${lann}`).then(res => res.json());
  ...
  if (command == 'npmstalk') {
    ...
    try {
        let npm = await fetch(`https://api.betabotz.eu.org/fire/stalk/npm?name=${text}&apikey=${lann}`).then(res => res.json());      
 ...
 if (command == 'repostalk') {
    ...
    try {
      let repo = await fetch(`https://api.betabotz.eu.org/fire/stalk/repo?repo=${text}&apikey=${lann}`).then(res => res.json());
