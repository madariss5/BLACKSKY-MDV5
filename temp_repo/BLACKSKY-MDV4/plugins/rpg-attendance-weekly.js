...
let handler = async (m, { isPrems }) => {
  ...
  if (new Date - global.db.data.users[m.sender].lastweekly < 604800000) throw `You have already mengklaim, klaim weeksan ini\ntunggu seold ${msToTime(time - new Date())} again`
...
function msToTime(duration) {
  ...
  return weeks + " days " +  hours + " hours " + minutes + " minutes"
}