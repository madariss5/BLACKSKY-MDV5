...
let handler = async (m, { conn, text }) => {
    ...
    const lang = user?.language || chat?.language || global.language;
 {
  ...
  if (m.chat in conn.hartatahta) throw 'Someone is already creating\nHarta Tahta Text\nin this chat... Please wait until it\'s finished'
  else conn.hartatahta[m.chat] = true
  m.reply('_Creating..._\n*Please wait*')
  try {
    let img = await ht(text ? text : 'your title')
    conn.sendFile(m.chat, img, 'Harta Tahta.png', '*Here you go*', m)
    setTimeout(() => {
      fs.unlinkSync(img)
    }, 5000);
  } finally {
