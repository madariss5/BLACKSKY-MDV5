...
async function downloadTikTok(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/tiktok?url=${link}&apikey=${lann}`);
			...
			conn.reply(m.chat, 'limit you habis!', m);
		}
		return;
	} catch (error) {
		console.error(error);
...
async function downloadDouyin(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/douyin?url=${link}&apikey=${lann}`);
			const data = await response.json();
			if (!data.result.video) {
				return;
			}
...
// 	try {
//         if (global.db.data.users[m.sender].limit > 0) {
// 	    const response = await fetch(`https://api.betabotz.eu.org/fire/download/terabox?url=${link}&apikey=${btc}`);        
...
async function downloadpin(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/pinterest?url=${link}&apikey=${lann}`);
...
async function downloadyt(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await axios.get(`https://api.betabotz.eu.org/fire/download/ytmp4?url=${link}&apikey=${lann}`);
...
async function downloadInstagram(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/igdowloader?url=${link}&apikey=${lann}`);
...
async function downloadFacebook(link, m) {
	try {
		if (global.db.data.users[m.sender].limit > 0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/fbdown?url=${link}&apikey=${lann}`);
...
async function _spotify(link, m) {
	try {
		if (global.db.data.users[m
			.sender].limit >
			0) {
			const res = await fetch(`https://api.betabotz.eu.org/fire/download/spotify?url=${link}&apikey=${lann}`)
			global.db.data.users[m.sender].limit -= 1
...
async function _twitter(link, m) {
	try {
		if (global.db.data.users[m
			.sender].limit >
			0) {
			const fire = await fetch(`https://api.betabotz.eu.org/fire/download/twitter2?url=${link}&apikey=${lann}`);
			global.db.data.users[m.sender].limit -= 1
			...
			for (const url of mediaURLs) {
				...
				await delay(3000)//3 seconds jeda agar not spam
...
async function _threads(link, m) {
	try {
		if (global.db.data.users[m
			.sender].limit >
			0) {
			const fire = await fetch(`https://api.betabotz.eu.org/fire/download/threads?url=${link}&apikey=${lann}`).then(results => results.json());
...
async function _capcut(link, m) {
	try {
		if (global.db.data.users[m
			.sender].limit >
			0) {
			const response = await fetch(`https://api.betabotz.eu.org/fire/download/capcut?url=${link}&apikey=${lann}`);
			global.db.data.users[m.sender].limit -= 1
			if (!response.ok) {
