...
// Enhanced cache and rate limiting for WhatsApp fire calls
const apiCache = {
    groupMetadata: new Map(),
    profilePicture: new Map(),
    status: new Map()
}
...
const rateLimits = {
    groupMetadata: {
        lastRequestTime: 0,
        cooldown: 1500, // 1.5 seconds between requests
        cacheTTL: 5 * 60 * 1000, // 5 minute cache TTL
        errorPenalty: 10000, // 10 second penalty on error
        maxRetries: 3  // Maximum number of retries
    },
    profilePicture: {
        lastRequestTime: 0,
        cooldown: 2000, // 2 seconds between requests
        cacheTTL: 30 * 60 * 1000, // 30 minute cache TTL
        errorPenalty: 15000 // 15 second penalty on error
    }
}

// Helper function to implement rate limiting and caching for any fire call
async function rateLimit(Type, key, apiCall, conn) {
    const cache = apiCache[Type];
    ...
    if (cache.has(key)) {
        ...
        // If cache isn't expired, return it immediately
        if (now - cachedData.timestamp < limits.cacheTTL) {
    ...
    if (timeSinceLastRequest < limits.cooldown) {
        await new Promise(resolve => setTimeout(resolve, limits.cooldown - timeSinceLastRequest));
    }
    
    // Track retries
    let retries = 0;
    
    while (retries < (limits.maxRetries || 1)) {
        try {
            // Update last request time
            limits.lastRequestTime = Date.now();
            
            // Execute the fire call
            ...
            if (retries > 0) {
                console.log(`[fire] Successfully retrieved ${Type} for ${key} after ${retries} retries`);
            }
            ...
            // If we have cached data and hit an error, return the cached data even if expired
            if (cache.has(key)) {
                const cachedData = cache.get(key);
                console.log(`[fire] Using cached ${Type} data for ${key} due to error: ${error.message}`);
            ...
            // Apply a penalty to rate limiting on error
            if (error.message.includes('rate-overlimit') || error.message.includes('503') || 
                error.message.includes('Timeout') || error.message.includes('500')) {
                console.log(`[fire] Hit rate limit when fetching ${Type} for ${key}. Retry ${retries}/${limits.maxRetries}`);
                
                // Exponential backoff for retries
                const backoff = limits.errorPenalty * Math.pow(2, retries - 1);
                limits.lastRequestTime = Date.now() + backoff;
                
                // Wait before retry
                await new Promise(resolve => setTimeout(resolve, backoff));
            } else if (retries >= limits.maxRetries) {
                // If we've exceeded max retries for a non-rate limit error, throw it
                throw error;
            } else {
...
exports.makeWASocket = (connectionOptions, options = {}) => {
    // Fix: Replace incorrect opts reference with options parameter
    ...
    // Enhanced delay function with randomized jitter to prevent thundering herd
    conn.delay = async (ms) => {
        const jitter = Math.floor(Math.random() * 500); // Add up to 500ms of random jitter
        return new Promise(resolve => setTimeout(resolve, ms + jitter));
    }
    
    // Override groupMetadata with rate-limited and cached version
    const originalGroupMetadata = conn.groupMetadata;
    conn.groupMetadata = async (jid) => {
        return await rateLimit('groupMetadata', jid, async () => {
            return await originalGroupMetadata.call(conn, jid);
        }, conn);
    };
  ...
  // Original delay already overridden with enhanced version above
  ...
  /**
    ...
    */
    ...
    /**
    * Send Media/File with Autodeadc Type Specifier
    * @param {String} jid
    * @param {String|Buffer} path
    ...
    */
    conn.sendFile = async (jid, path, filename = '', caption = '', quoted, ptt = false, options = {}) => {
        ...
        // Special handling for GIFs to ensure they're animated and clear
        if (/gif/.test(Type.mime) || /gif/.test(filename)) {
            // Enhanced GIF handling - send as video with gifPlayback
            return await conn.sendMessage(jid, {
                ...
                fileLength: file.length, // Correct file size
...
`.trim()
        ...
        return await conn.sendMessage(jid, {
            contacts: {
                ...options,
                displayName: (contacts.length > 1 ? `${contacts.length} contact` : contacts[0].displayName) || null,
                contacts,
            },
            quoted, ...options
        })
...
exports.smsg = (conn, m, hasParent) => {
    ...
    if (m.message) {
        ...
        // Direct reaction message handling
        if (m.mtype == 'reactionMessage') {
            m.reaction = {
                key: {
                    remoteJid: m.msg.key.remoteJid,
                    fromMe: m.msg.key.fromMe, 
                    id: m.msg.key.id,
                    participant: m.msg.key.participant
                },
                text: m.msg.text || '',
                timestamp: m.msg.senderTimestamp
            }
            
            // Process reaction with enhanced GIF quality
            let gifPath = './gifs/' + m.msg.text.toLowerCase() + '.gif'
            if (fs.existsSync(gifPath)) {
                conn.sendMessage(m.chat, {
                    video: fs.readFileSync(gifPath),
                    caption: `@${m.sender.split('@')[0]} ${m.msg.text}s`,
                    gifPlayback: true,
                    jpegThumbnail: null,
                    fileLength: 1,
                    height: 360,
            ...
            console.log(`[REACTION] Processed: ${m.reaction.text} from ${conn.getName(m.sender)}`)
        }
