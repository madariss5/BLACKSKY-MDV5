...
handler.before = async function (m) {
    let id = m.chat
    if (!m.quoted || !m.quoted.fromMe || !m.quoted.isBaileys || !m.text || !/Type.*hemo/i.test(m.quoted.text) || /.*hemo/i.test(m.text))
        return !0
    this.tebakemoji = this.tebakemoji ? this.tebakemoji : {}
    if (!(id in this.tebakemoji))
        return this.reply(m.chat, 'Soal that has berakhir', m)
