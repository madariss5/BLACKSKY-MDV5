...
await conn.sendPresenceUpdate('composing', m.chat)
    ...
    if (!text) {
        var message = "Harap aktif di group karena will ada pemcleanan member setiap saat"
    } else {
        var message = text
    }
    var sum
    ...
    if (total == 0) return conn.reply(m.chat, `*Digroup this not terable to sider.*`, m)
    conn.reply(m.chat, `*${total}/${sum}* anggota group *${await conn.getName(m.chat)}* adalah sider dengan alasan :\n1. Tidak aktif seold lebih dari 7 days\n2. Baru join tetapi not pernah nimbrung\n\n_“${message}”_\n\n*LIST SIDER :*\n${sider.map(v => '  ○ @' + v.replace(/@.+/, '' + typeof global.db.data.users[v] == "undefined" ? ' Sider ' : ' Off ' + msToDate(milliseconds * 1 - global.db.data.users[v].lastseen))).join('\n')}`, m, {
        contextInfo: {
            mentionedJid: sider
        }
    })
