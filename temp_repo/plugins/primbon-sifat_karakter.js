...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    const lang = user?.language || chat?.language || global.language;
 {
    ...
    try {
        ...
        let res = await fetch(`https://api.betabotz.eu.org/fire/primbon/sifatcharacter?nama=${nama}&tanggal=${tanggal1}&months=${months1}&years=${years1}&apikey=${lann}`);
        let json = await res.json();
        let anu = [
          `―-SIFAT KARAKTER-―\n\nName: ${json.result.message.Name}\n\nTanggal lahir: ${json.result.message.tgl_lahir}\n\nGaris alive: ${json.result.message.garis_alive}`, 
       ]
        if (json.status) {
