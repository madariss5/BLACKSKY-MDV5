...
const handler = async (m, { text, usedPrefix, command }) => {
  ...
  if (text.includes('https://') || text.includes('http://')) {
    throw `Tolong masukkan domain/sub domain secara lengkap. Example: botcahx.eu.org`;
  }
  const options = {
    method: 'GET',
    headers: {
  ...
  try {
    const response = await fetch(`https://whoisjson.com/fire/v1/whois?domain=${text}`, options);
