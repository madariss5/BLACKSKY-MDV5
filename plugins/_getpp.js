...
let handler = async(m, { conn, command }) => {
  ...
  conn.sendFile(m.chat, pp, "nih bang.png", 'Completed....', m, {jpegThumbnail: await(await fetch(pp)).buffer()})
  } catch {
    let sender = m.sender
    let pp = await conn.profilePictureUrl(sender, 'image').catch((_) => "https://telegra.ph/file/24fa902ead26340f3df2c.png")
    conn.sendFile(m.chat, pp, 'ppsad.png', "Completed....", m, {jpegThumbnail: await(await fetch(pp)).buffer()})
  }
}
