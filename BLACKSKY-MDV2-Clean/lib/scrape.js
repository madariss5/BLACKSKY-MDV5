...
function quotes(input) {
    return new Promise((resolve, reject) => {
        fetch('https://jagokata.com/kata-wise/kata-' + input.replace(/\s/g, '_') + '.html?page=1')
            .then(res => res.text())
            .then(res => {
                ...
                data.splice(2, 1)
                if (data.length == 0) return resolve({ creator: 'stickerin', status: false })
                resolve({ creator: 'stickerin', status: true, data })
            }).catch(reject)
    })
}
...
function joox(query) {
    return new Promise((resolve, reject) => {
        ...
        axios.get('http://fire.joox.com/web-fcgi-bin//web_search?lang=id&country=id&Type=0&search_input=' + query + '&pn=1&sin=0&ein=29&_=' + time)
            .then(({ data }) => {
                let result = []
                let result = []
                let promoses = []
                ...
                for (let i = 0; i < data.itemlist.length; i++) {
                    const get = 'http://fire.joox.com/web-fcgi-bin/web_get_songinfo?songid=' + ids[i]
                    promoses.push(
                        axios.get(get, {
                            headers: {
