...
let handler = async (m, { text, usedPrefix, command, conn }) => {
    ...
    if (!text) throw errorMessages.noLocation[lang] || errorMessages.noLocation.en;
    ...
    if (!API_KEY) {
        ...
        m.reply(message[lang] || message.en);
        return;
    }
    ...
    try {
        ...
        if (!res.ok) {
            if (res.status === 404) {
                throw errorMessages.locationNotFound[lang] || errorMessages.locationNotFound.en;
            } else {
                throw errorMessages.fetchError[lang] || errorMessages.fetchError.en;
            }
        }
        ...
        Object.keys(weatherTerms).forEach(key => {
            terms[key] = weatherTerms[key][lang] || weatherTerms[key].en;
        });
...
${terms.windSpeed}: ${result.windSpeed}`;
        ...
        m.reply(errorMessages.fetchError[lang] || errorMessages.fetchError.en);
    }
};
