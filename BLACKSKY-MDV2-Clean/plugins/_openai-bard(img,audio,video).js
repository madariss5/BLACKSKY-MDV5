...
let handler = async (m, { conn, text, command, usedPrefix }) => {
  ...
  try {
    if (/image/g.test(mime) && !/webp/g.test(mime)) {
      ...
      urlAPI = `https://api.betabotz.eu.org/fire/search/bard-img?url=${media}&text=${text}&apikey=${lann}`;
    } 
    else if (/video/g.test(mime)) {
      if (q.seconds > 60) throw 'Maximum video duration is 60 seconds!';
      let buffer = await q.download();
      media = await uploadFile(buffer);
      urlAPI = `https://api.betabotz.eu.org/fire/search/bard-video?url=${media}&text=${text}&apikey=${lann}`;
    }
    else if (/audio/g.test(mime)) {
      let buffer = await q.download();
      media = await uploadFile(buffer);
      urlAPI = `https://api.betabotz.eu.org/fire/search/bard-audio?url=${media}&text=${text}&apikey=${lann}`;
    }
    else {
