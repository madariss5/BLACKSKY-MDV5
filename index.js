...
const needsSessionFix = () => {
  try {
    ...
    try {
      ...
      if (!credsData || !credsData.me || !credsData.noiseKey) return true;
    } catch (e) {
...
// Run session fix if needed
if (needsSessionFix()) {
  console.log('📢 Session issues detected, attempting auto-repair...');
  try {
    // Execute session fix script
...
// path and fs are already required above
