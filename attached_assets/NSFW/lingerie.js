...
let handler = async (m, { conn, isAdmin, isOwner }) => {
  ...
  m.reply('🔍 *Searching for lingerie content...*');
  
  try {
    // Try to get images from API
    const response = await axios.get('https://api.waifu.pics/nsfw/lingerie', { 
      timeout: 10000,
    ...
    console.error('Lingerie command error:', error);
    m.reply('❌ Failed to fetch content. Please try again later.');
  }
};

handler.help = ['lingerie'];
handler.tags = ['nsfw'];
handler.command = /^lingerie$/i;
